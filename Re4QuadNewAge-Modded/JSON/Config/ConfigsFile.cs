...
namespace Re4QuadExtremeEditor.src.JSON
{
    static class ConfigsFile
    {
        ...
        public static void writeConfigsFile(string filename, Configs config)
        {
            ...
            entry["IncludeAEV"] = config.IncludeAEV;
            entry["IncludeDSE"] = config.IncludeDSE;
            entry["IncludeEAR"] = config.IncludeEAR;
            entry["IncludeEMI"] = config.IncludeEMI;
            entry["IncludeESE"] = config.IncludeESE;
            entry["IncludeETS"] = config.IncludeETS;
            entry["IncludeFSE"] = config.IncludeFSE;
            entry["IncludeITA"] = config.IncludeITA;
            entry["IncludeLIT"] = config.IncludeLIT;
            entry["IncludeSAR"] = config.IncludeSAR;
            entry["UseDataQingshengSource"] = config.UseDataQingshengSource;
            ...
            entry["UseLightTheme"] = config.UseLightTheme;
            entry["UseInvertedMouseButtons"] = config.UseInvertedMouseButtons;
            entry["SetupDone"] = config.SetupDone;
            entry["LoadLangTranslation"] = config.LoadLangTranslation;
            entry["LangJsonFile"] = config.LangJsonFile;

            entry["FpsLimit"] = config.FpsLimit;
            entry["RecentFiles"] = new JArray((config.RecentFiles ?? new List<string>()).ToArray());
        ...
        public static Configs parseConfigs(string filename)
        {
            ...
            if (File.Exists(filename))
            {
                ...
                if (o["Configs"] != null)
                {
                    ...
                    // optional entries: keep defaults when missing
                    if (oConfigs["ToolPathUDAS"] != null) config.ToolPathUDAS = oConfigs["ToolPathUDAS"].ToString();
                    if (oConfigs["ToolPathLFS"] != null) config.ToolPathLFS = oConfigs["ToolPathLFS"].ToString();
                    if (oConfigs["ToolPathPACK"] != null) config.ToolPathPACK = oConfigs["ToolPathPACK"].ToString();
                    if (oConfigs["ToolPathGCA"] != null) config.ToolPathGCA = oConfigs["ToolPathGCA"].ToString();

                    try { config.IncludeAEV = bool.Parse(oConfigs["IncludeAEV"].ToString()); } catch (Exception) { }
                    try { config.IncludeDSE = bool.Parse(oConfigs["IncludeDSE"].ToString()); } catch (Exception) { }
                    try { config.IncludeEAR = bool.Parse(oConfigs["IncludeEAR"].ToString()); } catch (Exception) { }
                    try { config.IncludeEMI = bool.Parse(oConfigs["IncludeEMI"].ToString()); } catch (Exception) { }
                    try { config.IncludeESE = bool.Parse(oConfigs["IncludeESE"].ToString()); } catch (Exception) { }
                    try { config.IncludeETS = bool.Parse(oConfigs["IncludeETS"].ToString()); } catch (Exception) { }
                    try { config.IncludeFSE = bool.Parse(oConfigs["IncludeFSE"].ToString()); } catch (Exception) { }
                    try { config.IncludeITA = bool.Parse(oConfigs["IncludeITA"].ToString()); } catch (Exception) { }
                    try { config.IncludeLIT = bool.Parse(oConfigs["IncludeLIT"].ToString()); } catch (Exception) { }
                    try { config.IncludeSAR = bool.Parse(oConfigs["IncludeSAR"].ToString()); } catch (Exception) { }
                    ...
                    if (oConfigs["UseDataQingshengSource"] != null)
                    {
                        try
                        {
                            config.UseDataQingshengSource = bool.Parse(oConfigs["UseDataQingshengSource"].ToString());
                        }
                        catch (Exception)
                        {
                        }
                    ...
                    if (oConfigs["SetupDone"] != null)
                    {
                        try
                        {
                            config.SetupDone = bool.Parse(oConfigs["SetupDone"].ToString());
                        }
                        catch (Exception)
                        {
                        }
                    ...
                    if (oConfigs["UseLightTheme"] != null)
                    {
                        try
                        {
                            config.UseLightTheme = bool.Parse(oConfigs["UseLightTheme"].ToString());
                        }
                        catch (Exception)
                        {
                        }
                    ...
                    if (oConfigs["FpsLimit"] != null)
                    {
                        try
                        {
                            config.FpsLimit = int.Parse(oConfigs["FpsLimit"].ToString(), System.Globalization.NumberStyles.Number);
                        }
                        catch (Exception)
                        {
                        }
                    ...
                    if (oConfigs["RecentFiles"] != null && oConfigs["RecentFiles"] is JArray recentArray)
                    {
                        var stored = new List<string>();
                        foreach (JToken token in recentArray)
                        {
                            stored.Add(token.ToString());
                        }
                        config.RecentFiles = stored;
                    }
