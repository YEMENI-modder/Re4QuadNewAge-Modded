...
namespace Re4QuadExtremeEditor.src.Controls
{
    public partial class ObjectMoveControl : UserControl
    {
        ...
        public void UpdateSelection()
        {
            ...
            if (SelectedNodes.Count > 0)
            {
                for (int i = 0; i < SelectedNodes.Count; i++)
                {
                    if (SelectedNodes[i] is Object3D obj)
                    {
                        ...
                        else if (parent is NewAge_CAM_NodeGroup)
                        {
                            combos |= MoveObjCombos.Enemy;
                        }
                        else if (parent is NewAge_CAM_Zone_NodeGroup)
                        {
                            combos |= MoveObjCombos.TriggerZone;
                        }
                        else if (parent is NewAge_RTP_NodeGroup)
                        {
                            combos |= MoveObjCombos.RtpNode;
                        }
                    }
                }
            ...
            else if (combos == MoveObjCombos.RtpNode)
            {
                comboBoxMoveMode.Items.Add(new MoveObjTypeObjForListBox(MoveObjType.SquareMoveObjXZ_VerticalMoveObjY_Horizontal123None, Lang.GetText(eLang.MoveMode_RTP_PositionPoint)));
            }
        ...
        private void checkBoxEnableGizmo_CheckedChanged(object sender, EventArgs e)
        {
            Class.Gizmo.Enabled = checkBoxEnableGizmo.Checked;
        }
        ...
        // ------------------------------------------------------------------
        // Undo support for panel-driven moves: capture the COMPLETE state
        // (position + rotation + scale) when a pad press starts a gesture,
        // push ONE command on release. Covers enemies (rotation), trigger
        // zones (resize via points), etcmodel (scale), everything.
        private System.Collections.Generic.List<Object3D> undoObjects;
        private System.Collections.Generic.List<UndoSystem.FullTransformState> undoStartStates;

        private void UndoBegin()
        {
            undoObjects = new System.Collections.Generic.List<Object3D>();
            undoStartStates = new System.Collections.Generic.List<UndoSystem.FullTransformState>();
            foreach (TreeNode item in DataBase.SelectedNodes.Values)
            {
                if (item is Object3D obj && obj.Parent != null)
                {
                    try
                    {
                        undoStartStates.Add(UndoSystem.CaptureFullTransform(obj));
                        undoObjects.Add(obj);
                    }
                    catch { }
                }
        ...
        {
            var objs = undoObjects;
            var starts = undoStartStates;
            undoObjects = null;
            undoStartStates = null;
            if (objs == null || objs.Count == 0 || starts == null || objs.Count != starts.Count) return;

            var capturedObjs = objs;
            UndoSystem.PushFullTransform(objs, starts,
                delegate ()
                {
                    UndoSystem.FullTransformState[] cur = new UndoSystem.FullTransformState[capturedObjs.Count];
                    for (int i = 0; i < cur.Length; i++)
                    {
                        if (capturedObjs[i] != null && capturedObjs[i].Parent != null)
                            cur[i] = UndoSystem.CaptureFullTransform(capturedObjs[i]);
                    }
                    return cur;
                }, null);
        }
