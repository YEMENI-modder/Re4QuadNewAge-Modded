   ...
   public static class Lang
    {
        ...
        private static Dictionary<eLang, string> englishDefaults;
        private static Dictionary<aLang, string> englishAttrDefaults;
        private static bool othersTextsStarted;

        public static void StartOthersTextsSafe()
        {
            if (!othersTextsStarted)
            {
                othersTextsStarted = true;
                StartOthersTexts();
            }
        }

        public static void SnapshotEnglishDefaults()
        {
            if (englishDefaults == null)
            {
                englishDefaults = new Dictionary<eLang, string>(Text);
                englishAttrDefaults = new Dictionary<aLang, string>(AttributeText);
            }
        }

        public static void RestoreEnglishDefaults()
        {
            if (englishDefaults == null) { return; }
            List<eLang> removeTexts = Text.Keys.Where(k => !englishDefaults.ContainsKey(k)).ToList();
            foreach (eLang key in removeTexts)
            {
                Text.Remove(key);
            }
            foreach (KeyValuePair<eLang, string> kv in englishDefaults)
            {
                Text[kv.Key] = kv.Value;
            }
            if (englishAttrDefaults != null)
            {
                List<aLang> removeAttrs = AttributeText.Keys.Where(k => !englishAttrDefaults.ContainsKey(k)).ToList();
                foreach (aLang key in removeAttrs)
                {
                    AttributeText.Remove(key);
                }
                foreach (KeyValuePair<aLang, string> kv in englishAttrDefaults)
                {
                    AttributeText[kv.Key] = kv.Value;
                }
            }
        }
        ...
        public static void StartAttributeTexts()
        {
            ...
            AttributeText.Add(aLang.Enemy_AssociatedITAItemCategory, "Associated ITA Item");
            AttributeText.Add(aLang.Enemy_AssociatedITAItemDisplayName, "ITA Entry");
            AttributeText.Add(aLang.Enemy_AssociatedITAItemDescription, "Select an ITA entry to associate with this enemy. Selecting an entry automatically sets Ref Interaction Type to Enemy and Ref Interaction Index to this enemy Order ID.");
            AttributeText.Add(aLang.Enemy_AssociatedITARefInteractionTypeDisplayName, "Ref Interaction Type (List)");
            AttributeText.Add(aLang.Enemy_AssociatedITARefInteractionIndexDisplayName, "Ref Interaction Index [Hex] (Byte)");
            AttributeText.Add(aLang.Enemy_AssociatedITAItemNumberDisplayName, "Item Number [Hex] (Ushort)");
            AttributeText.Add(aLang.Enemy_AssociatedITASecundIndexDisplayName, "Secund Index [Hex] (Ushort)");
            AttributeText.Add(aLang.Enemy_AssociatedITAItemAuraTypeDisplayName, "Item Aura Type [Hex] (Ushort)");
            ...
            //SMX Property
            AttributeText.Add(aLang.NewAge_SMX_Category, "SMX");

            AttributeText.Add(aLang.SMX_UseSMXID_Byte_DisplayName, "Use SMX ID (Byte)");
            AttributeText.Add(aLang.SMX_UseSMXID_Byte_Description, "Offset[0x00] (Byte) \r\n0x00 = disabled, 0x01 = enabled");

            AttributeText.Add(aLang.SMX_Mode_Byte_DisplayName, "Mode (Byte)");
            AttributeText.Add(aLang.SMX_Mode_Byte_Description, "Offset[0x01] (Byte) \r\n0x00 = Normal, 0x01 = Rotate, 0x02 = Swing");

            AttributeText.Add(aLang.SMX_OpacityHierarchy_Byte_DisplayName, "Opacity Hierarchy (Byte)");
            AttributeText.Add(aLang.SMX_OpacityHierarchy_Byte_Description, "Offset[0x02] (Byte) \r\nOpacity hierarchy value");

            AttributeText.Add(aLang.SMX_FaceCulling_Byte_DisplayName, "Face Culling (Byte)");
            AttributeText.Add(aLang.SMX_FaceCulling_Byte_Description, "Offset[0x03] (Byte) \r\nFace culling mode");

            AttributeText.Add(aLang.SMX_LightSwitch_Uint32_DisplayName, "Light Switch [Hex] (Uint32)");
            AttributeText.Add(aLang.SMX_LightSwitch_Uint32_Description, "Offset[0x04]-[0x07] (UInt32) \r\nBitmask for light sources");

            AttributeText.Add(aLang.SMX_AlphaHierarchy_Byte_DisplayName, "Alpha Hierarchy (Byte)");
            AttributeText.Add(aLang.SMX_AlphaHierarchy_Byte_Description, "Offset[0x08] (Byte) \r\nAlpha hierarchy value");

            AttributeText.Add(aLang.SMX_ColorRGB_DisplayName, "Color RGB [Hex] (3 Bytes)");
            AttributeText.Add(aLang.SMX_ColorRGB_Description, "Offset[0x0C]-[0x0E] (3 Bytes) \r\nR,G,B in hex, e.g. FF00FF");

            AttributeText.Add(aLang.SMX_ColorAlpha_Byte_DisplayName, "Color Alpha (Byte)");
            AttributeText.Add(aLang.SMX_ColorAlpha_Byte_Description, "Offset[0x0F] (Byte) \r\nAlpha channel");

            AttributeText.Add(aLang.SMX_SpecularColor_Uint32_DisplayName, "Specular Color [Hex] (Uint32)");
            AttributeText.Add(aLang.SMX_SpecularColor_Uint32_Description, "Offset[0x84]-[0x87] (UInt32) \r\nSpecular color value");

            AttributeText.Add(aLang.SMX_TextureMovement_X_Float_DisplayName, "Texture Movement X (Float)");
            AttributeText.Add(aLang.SMX_TextureMovement_X_Float_Description, "Offset[0x88]-[0x8B] (Float) \r\nTexture scroll X speed");

            AttributeText.Add(aLang.SMX_TextureMovement_Y_Float_DisplayName, "Texture Movement Y (Float)");
            AttributeText.Add(aLang.SMX_TextureMovement_Y_Float_Description, "Offset[0x8C]-[0x8F] (Float) \r\nTexture scroll Y speed");

            AttributeText.Add(aLang.SMX_RotationSpeed_X_Float_DisplayName, "Rotation Speed X (Float)");
            AttributeText.Add(aLang.SMX_RotationSpeed_X_Float_Description, "Offset[0x10]-[0x13] (Float) \r\nMode 0x01 Rotate X speed");

            AttributeText.Add(aLang.SMX_RotationSpeed_Y_Float_DisplayName, "Rotation Speed Y (Float)");
            AttributeText.Add(aLang.SMX_RotationSpeed_Y_Float_Description, "Offset[0x14]-[0x17] (Float) \r\nMode 0x01 Rotate Y speed");

            AttributeText.Add(aLang.SMX_RotationSpeed_Z_Float_DisplayName, "Rotation Speed Z (Float)");
            AttributeText.Add(aLang.SMX_RotationSpeed_Z_Float_Description, "Offset[0x18]-[0x1B] (Float) \r\nMode 0x01 Rotate Z speed");

            AttributeText.Add(aLang.SMX_Swing0_Float_DisplayName, "Swing 0 (Float)");
            AttributeText.Add(aLang.SMX_Swing0_Float_Description, "Offset[0x10]-[0x13] (Float) \r\nMode 0x02 Swing parameter 0");
            AttributeText.Add(aLang.SMX_Swing1_Float_DisplayName, "Swing 1 (Float)");
            AttributeText.Add(aLang.SMX_Swing1_Float_Description, "Offset[0x14]-[0x17] (Float) \r\nMode 0x02 Swing parameter 1");
            AttributeText.Add(aLang.SMX_Swing2_Float_DisplayName, "Swing 2 (Float)");
            AttributeText.Add(aLang.SMX_Swing2_Float_Description, "Offset[0x18]-[0x1B] (Float) \r\nMode 0x02 Swing parameter 2");
            AttributeText.Add(aLang.SMX_Swing3_Float_DisplayName, "Swing 3 (Float)");
            AttributeText.Add(aLang.SMX_Swing3_Float_Description, "Offset[0x1C]-[0x1F] (Float) \r\nMode 0x02 Swing parameter 3");
            AttributeText.Add(aLang.SMX_Swing4_Float_DisplayName, "Swing 4 (Float)");
            AttributeText.Add(aLang.SMX_Swing4_Float_Description, "Offset[0x20]-[0x23] (Float) \r\nMode 0x02 Swing parameter 4");
            AttributeText.Add(aLang.SMX_Swing5_Float_DisplayName, "Swing 5 (Float)");
            AttributeText.Add(aLang.SMX_Swing5_Float_Description, "Offset[0x24]-[0x27] (Float) \r\nMode 0x02 Swing parameter 5");
            AttributeText.Add(aLang.SMX_Swing6_Float_DisplayName, "Swing 6 (Float)");
            AttributeText.Add(aLang.SMX_Swing6_Float_Description, "Offset[0x28]-[0x2B] (Float) \r\nMode 0x02 Swing parameter 6");
            AttributeText.Add(aLang.SMX_Swing7_Float_DisplayName, "Swing 7 (Float)");
            AttributeText.Add(aLang.SMX_Swing7_Float_Description, "Offset[0x2C]-[0x2F] (Float) \r\nMode 0x02 Swing parameter 7");
            AttributeText.Add(aLang.SMX_Swing8_Float_DisplayName, "Swing 8 (Float)");
            AttributeText.Add(aLang.SMX_Swing8_Float_Description, "Offset[0x30]-[0x33] (Float) \r\nMode 0x02 Swing parameter 8");
            AttributeText.Add(aLang.SMX_Swing9_Float_DisplayName, "Swing 9 (Float)");
            AttributeText.Add(aLang.SMX_Swing9_Float_Description, "Offset[0x34]-[0x37] (Float) \r\nMode 0x02 Swing parameter 9");
            AttributeText.Add(aLang.SMX_SwingA_Float_DisplayName, "Swing A (Float)");
            AttributeText.Add(aLang.SMX_SwingA_Float_Description, "Offset[0x38]-[0x3B] (Float) \r\nMode 0x02 Swing parameter A");
            AttributeText.Add(aLang.SMX_SwingB_Float_DisplayName, "Swing B (Float)");
            AttributeText.Add(aLang.SMX_SwingB_Float_Description, "Offset[0x3C]-[0x3F] (Float) \r\nMode 0x02 Swing parameter B");
            AttributeText.Add(aLang.SMX_SwingC_Float_DisplayName, "Swing C (Float)");
            AttributeText.Add(aLang.SMX_SwingC_Float_Description, "Offset[0x40]-[0x43] (Float) \r\nMode 0x02 Swing parameter C");

            AttributeText.Add(aLang.SMX_Line_ByteArray_DisplayName, "Raw Data [Hex] (116 Bytes)");
            AttributeText.Add(aLang.SMX_Line_ByteArray_Description, "Offset[0x10]-[0x83] (116 Bytes) \r\nMode 0x00 Normal raw data block");

            //AVL Property
            AttributeText.Add(aLang.NewAge_AVL_Category, "AVL");

            AttributeText.Add(aLang.AVL_NumberOfAev_DisplayName, "Number of Aev [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.AVL_NumberOfAev_Description, "AEV event number linked to this door \r\nOffset[0x0F] (Byte) \r\nDropdown lists the loaded AEV events (if the AEV file is open) \r\nInput follows the current View > HEX/DEC mode; prefix 0x always means hex ");

            AttributeText.Add(aLang.AVL_KeyId_DisplayName, "Key ID [Hex/Dec] (Ushort)");
            AttributeText.Add(aLang.AVL_KeyId_Description, "Door Key ID value, 16-bit little endian \r\nOffset[0x1E][0x1F] (Ushort) \r\nDropdown lists all game items/keys (F2 = search) \r\nInput follows the current View > HEX/DEC mode; prefix 0x always means hex ");

            AttributeText.Add(aLang.AVL_LockMessage_DisplayName, "Lock Message [Hex/Dec]");
            AttributeText.Add(aLang.AVL_LockMessage_Description, "Message number shown when the door is locked (0xFF = none) \r\nOffset[0x2F] (Byte) \r\nInput: 0x03 = hex | 3 = decimal ");

            AttributeText.Add(aLang.AVL_LockSound_DisplayName, "Lock Sound [Hex/Dec]");
            AttributeText.Add(aLang.AVL_LockSound_Description, "Sound number played when the door is locked (0xFF = none) \r\nOffset[0x3F] (Byte) \r\nInput: 0x05 = hex | 5 = decimal ");

            AttributeText.Add(aLang.AVL_LockCameraEnabled_DisplayName, "Lock Camera Enabled");
            AttributeText.Add(aLang.AVL_LockCameraEnabled_Description, "True = fixed camera active (0x01), False = off (0xFF) \r\nOffset[0x4F] (Byte) ");

            AttributeText.Add(aLang.AVL_LockCamera_DisplayName, "Lock Camera Raw [Hex] (Byte)");
            AttributeText.Add(aLang.AVL_LockCamera_Description, "Lock camera raw value: 0xFF = off, 0x01 = on \r\nOffset[0x4F] (Byte) ");

            AttributeText.Add(aLang.AVL_UnlockMessage_DisplayName, "UnLock Message [Hex/Dec]");
            AttributeText.Add(aLang.AVL_UnlockMessage_Description, "Message number shown when the door is unlocked (0xFF = none) \r\nOffset[0x5F] (Byte) \r\nInput: 0x03 = hex | 3 = decimal ");

            AttributeText.Add(aLang.AVL_UnlockSound_DisplayName, "UnLock Sound [Hex/Dec]");
            AttributeText.Add(aLang.AVL_UnlockSound_Description, "Sound number played when the door is unlocked (0xFF = none) \r\nOffset[0x6F] (Byte) \r\nInput: 0x05 = hex | 5 = decimal ");

            //CAM Property
            AttributeText.Add(aLang.NewAge_RTP_Category, "RTP Node");
            AttributeText.Add(aLang.NewAge_RTPLinks_Category, "Route Links");
            AttributeText.Add(aLang.RTP_PosX_DisplayName, "Position X (Raw)");
            AttributeText.Add(aLang.RTP_PosY_DisplayName, "Position Y (Raw)");
            AttributeText.Add(aLang.RTP_PosZ_DisplayName, "Position Z (Raw)");
            AttributeText.Add(aLang.RTP_Pos_Description, "Waypoint position in raw file units (divide by 100 for meters).\r\nSame world space as cameras and rooms. ");
            AttributeText.Add(aLang.RTP_DistanceTableIndex_DisplayName, "Distance Table Index");
            AttributeText.Add(aLang.RTP_DistanceTableIndex_Description, "Index of this node's first entry inside the global distance table.\r\nMaintained automatically when links are added/removed. ");
            AttributeText.Add(aLang.RTP_ConnectionCount_DisplayName, "Connection Count");
            AttributeText.Add(aLang.RTP_ConnectionCount_Description, "Number of links leaving this node.\r\nUse Connect To / Disconnect From to change it safely. ");
            AttributeText.Add(aLang.RTP_ConnectTo_DisplayName, "Connect To Node");
            AttributeText.Add(aLang.RTP_ConnectTo_Description, "Creates a two-way link between this node and the chosen waypoint,\r\nusing the real 3D distance between them, then rebuilds the routing matrix.\r\nPick \"Node 17\" from the dropdown, or just type 17 and press Enter\r\n(plain decimal numbers, same as the tree names). ");
            AttributeText.Add(aLang.RTP_DisconnectFrom_DisplayName, "Disconnect From Node");
            AttributeText.Add(aLang.RTP_DisconnectFrom_Description, "Removes the two-way link between this node and the chosen waypoint\r\n(both directions), then rebuilds the routing matrix.\r\nThe dropdown lists only the nodes this waypoint is currently linked to. ");
            AttributeText.Add(aLang.RTP_LinksSummary_DisplayName, "Links (readonly)");
            AttributeText.Add(aLang.RTP_LinksSummary_Description, "Current links of this node: target(distance) pairs read from the file's distance table. ");
            AttributeText.Add(aLang.NewAge_CAM_Category, "CAM");
            AttributeText.Add(aLang.NewAge_CAMKeys_Category, "CAM Keyframes");
            AttributeText.Add(aLang.NewAge_CAMZone_Category, "CAM Trigger Zone");
            AttributeText.Add(aLang.NewAge_CAMZoneLink_Category, "CAM Link");
            AttributeText.Add(aLang.NewAge_CAMPoints_Category, "CAM Zone Points");

            AttributeText.Add(aLang.CAM_CamId_DisplayName, "Camera ID [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.CAM_CamId_Description, "Internal reference ID stored in the file \r\nOffset[+0x01] (Byte) \r\nOriginal Capcom files may have non-sequential IDs; only 0 is considered uninitialized ");

            AttributeText.Add(aLang.CAM_Type_DisplayName, "Camera Type [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.CAM_Type_Description, "Camera behavior mode, valid values 0-8 \r\nOffset[+0x02] (Byte) \r\n0 = Locked Position & Rotation \r\n1 = Align Behind Player (FOV pull) \r\n2 = Overhead Locked \r\n3 = Overhead Locked (tracks player) \r\n4 = Position & Rotation + Pan \r\n5 = Locked Rotation + Player Position \r\n6 = Locked + Animated (Inspection) \r\n7 = Locked on TriggerZone Entry \r\n8 = Over The Shoulder ");

            AttributeText.Add(aLang.CAM_Flags_DisplayName, "Flags [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.CAM_Flags_Description, "Camera flags bitmask \r\nOffset[+0x03] (Byte) \r\n1=Normal 2=Battle 4=Event 8=Door 16=Once 32=Ahead 64=Direct 128=Dislgt \r\nRule: Event cannot combine with other bits; Door requires Ahead ");

            AttributeText.Add(aLang.CAM_FlagNormal_DisplayName, "Flag: Normal");
            AttributeText.Add(aLang.CAM_FlagBattle_DisplayName, "Flag: Battle");
            AttributeText.Add(aLang.CAM_FlagEvent_DisplayName, "Flag: Event");
            AttributeText.Add(aLang.CAM_FlagDoor_DisplayName, "Flag: Door");
            AttributeText.Add(aLang.CAM_FlagOnce_DisplayName, "Flag: Once");
            AttributeText.Add(aLang.CAM_FlagAhead_DisplayName, "Flag: Ahead");
            AttributeText.Add(aLang.CAM_FlagDirect_DisplayName, "Flag: Direct");
            AttributeText.Add(aLang.CAM_FlagDislgt_DisplayName, "Flag: Dislgt");
            AttributeText.Add(aLang.CAM_FlagBit_Description, "Individual bit of the Flags byte \r\nNormal=1 Battle=2 Event=4 Door=8 Once=16 Ahead=32 Direct=64 Dislgt=128 ");

            AttributeText.Add(aLang.CAM_Distance_DisplayName, "Distance (Float)");
            AttributeText.Add(aLang.CAM_Distance_Description, "Camera distance parameter \r\nOffset[+0x08] (Float) ");

            AttributeText.Add(aLang.CAM_Unk021_DisplayName, "Unknown +0x00 [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.CAM_Unk021_Description, "Unknown byte \r\nOffset[+0x00] (Byte), usually 0x01 ");

            AttributeText.Add(aLang.CAM_Unk025_DisplayName, "Unknown +0x04 [Hex/Dec] (Uint)");
            AttributeText.Add(aLang.CAM_Unk025_Description, "Unknown 32-bit value \r\nOffset[+0x04] (Uint) ");

            AttributeText.Add(aLang.CAM_Unk027_DisplayName, "Unknown +0x0C (Float)");
            AttributeText.Add(aLang.CAM_Unk027_Description, "Unknown float value \r\nOffset[+0x0C] (Float) ");

            AttributeText.Add(aLang.CAM_Raw12_DisplayName, "Raw Bytes +0x14 [Hex] (12 bytes)");
            AttributeText.Add(aLang.CAM_Raw12_Description, "Reserved raw bytes between Buf0 address and keyframe count \r\nOffset[+0x14..+0x1F] ");

            AttributeText.Add(aLang.CAM_KeyframeCount_DisplayName, "Keyframe Count");
            AttributeText.Add(aLang.CAM_KeyframeCount_Description, "Number of position/target/zoom/fov keyframes \r\nChanging it resizes all parallel buffers (new zoom = 0, new fov = 50) \r\nType 8 cameras normally use exactly 24 keyframes \r\nStored at offset[+0x20] on save ");

            AttributeText.Add(aLang.CAM_SelectedKeyframe_DisplayName, "Selected Keyframe");
            AttributeText.Add(aLang.CAM_SelectedKeyframe_Description, "Index of the keyframe being edited below (0-based) \r\nThe X/Y/Z fields read and write this keyframe ");

            AttributeText.Add(aLang.CAM_PosX_DisplayName, "Position X (Float)");
            AttributeText.Add(aLang.CAM_PosY_DisplayName, "Position Y - Height (Float)");
            AttributeText.Add(aLang.CAM_PosZ_DisplayName, "Position Z (Float)");
            AttributeText.Add(aLang.CAM_Pos_Description, "Camera position of the selected keyframe in game units \r\nBuffer1: count x 12 bytes (3 floats) ");

            AttributeText.Add(aLang.CAM_TargetX_DisplayName, "Target X (Float)");
            AttributeText.Add(aLang.CAM_TargetY_DisplayName, "Target Y - Height (Float)");
            AttributeText.Add(aLang.CAM_TargetZ_DisplayName, "Target Z (Float)");
            AttributeText.Add(aLang.CAM_Target_Description, "Look-at target of the selected keyframe in game units \r\nBuffer2: count x 12 bytes (3 floats) ");

            AttributeText.Add(aLang.CAM_Zoom_DisplayName, "Zoom (Float)");
            AttributeText.Add(aLang.CAM_Zoom_Description, "Zoom value of the selected keyframe \r\nBuffer3: count x 4 bytes (float) ");

            AttributeText.Add(aLang.CAM_Fov_DisplayName, "FOV (Float)");
            AttributeText.Add(aLang.CAM_Fov_Description, "Field of view in degrees of the selected keyframe, typical range 5-170 \r\nBuffer4: count x 4 bytes (float) ");

            AttributeText.Add(aLang.CAM_TimeFrame_DisplayName, "Speed");
            AttributeText.Add(aLang.CAM_TimeFrame_Description, "DATASET 5 (Camera Speed) of the selected keyframe \r\nValues must be strictly ascending, first one is 0 \r\nExample: Speed_01=0, Speed_02=30 -> camera plays over 30 frames \r\nBuffer0: count x 2 bytes (ushort), only present on Type 6 ");

            AttributeText.Add(aLang.ZN_LinkLine_DisplayName, "Link Line [Hex] (16 bytes)");
            AttributeText.Add(aLang.ZN_LinkLine_Description, "Raw Table1 link record: trigger type byte, unknown byte, three ushorts, then the two runtime addresses (managed automatically on save) ");

            AttributeText.Add(aLang.ZN_TriggerType_DisplayName, "Trigger Type [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.ZN_TriggerType_Description, "How this camera event is activated \r\nOffset[+0x00] of Table1 link (Byte) \r\n0x03 = Walk Into Zone | 0x04 = AEV Event (Inspect/Open) | 0x23 = Walk Into Zone Alt \r\nOther values seen: 0x00 0x01 0x3B 0x41 0x43 0x63 0x81 0x83 ");

            AttributeText.Add(aLang.ZN_LinkedCamera_DisplayName, "Linked Camera");
            AttributeText.Add(aLang.ZN_LinkedCamera_Description, "Which camera from the CAM list fires when this zone triggers \r\nDropdown lists every loaded camera with its type and keyframe count ");

            AttributeText.Add(aLang.ZN_LinkUnk012_DisplayName, "Link Unknown +0x01 [Hex/Dec]");
            AttributeText.Add(aLang.ZN_LinkUnk012_Description, "Unknown byte of the Table1 link \r\nOffset[+0x01] (Byte) ");

            AttributeText.Add(aLang.ZN_Unk015_DisplayName, "Link Unknown +0x02 [Hex/Dec] (Ushort)");
            AttributeText.Add(aLang.ZN_Unk015_Description, "Unknown ushort of the Table1 link \r\nOffset[+0x02][+0x03] ");

            AttributeText.Add(aLang.ZN_Unk016_DisplayName, "Link Unknown +0x04 [Hex/Dec] (Ushort)");
            AttributeText.Add(aLang.ZN_Unk016_Description, "Unknown ushort of the Table1 link \r\nOffset[+0x04][+0x05] ");

            AttributeText.Add(aLang.ZN_Unk017_DisplayName, "Link Unknown +0x06 [Hex/Dec] (Ushort)");
            AttributeText.Add(aLang.ZN_Unk017_Description, "Unknown ushort of the Table1 link \r\nOffset[+0x06][+0x07] ");

            AttributeText.Add(aLang.ZN_EntryNumber_DisplayName, "Entry Number [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.ZN_EntryNumber_Description, "Sequential counter stored in Table2, normally equals the entry position (1-based) \r\nOffset[+0x01] of Table2 (Byte) ");

            AttributeText.Add(aLang.ZN_Height_DisplayName, "Height (Float)");
            AttributeText.Add(aLang.ZN_Height_Description, "Upper limit of the zone volume relative to Bottom \r\nOffset[+0x20] of Table2 (Float) \r\nTop plane Y = Points Y + Height ");

            AttributeText.Add(aLang.ZN_Bottom_DisplayName, "Bottom (Float)");
            AttributeText.Add(aLang.ZN_Bottom_Description, "Lower limit of the zone volume \r\nOffset[+0x24] of Table2 (Float) ");

            AttributeText.Add(aLang.ZN_CamTypeTz_DisplayName, "TZ Unknown +0x02 [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.ZN_CamTypeTz_Description, "Unknown byte of Table2 \r\nOffset[+0x02] (Byte) ");

            AttributeText.Add(aLang.ZN_Subtype_DisplayName, "Sub Type [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.ZN_Subtype_Description, "Trigger subtype byte of Table2, original files usually store 0x03 \r\nOffset[+0x03] (Byte) ");

            AttributeText.Add(aLang.ZN_Unk051_DisplayName, "TZ Unknown +0x00 [Hex/Dec] (Byte)");
            AttributeText.Add(aLang.ZN_Unk051_Description, "Unknown first byte of Table2, usually 0x01 \r\nOffset[+0x00] (Byte) ");

            AttributeText.Add(aLang.ZN_PointCount_DisplayName, "Point Count");
            AttributeText.Add(aLang.ZN_PointCount_Description, "Number of polygon points of the zone footprint \r\nMinimum 3 points; changing it pads or removes points \r\nStored as coord_count at offset[+0x28] on save ");

            AttributeText.Add(aLang.ZN_SelectedPoint_DisplayName, "Selected Point");
            AttributeText.Add(aLang.ZN_SelectedPoint_Description, "Index of the polygon point being edited below (0-based) ");

            AttributeText.Add(aLang.ZN_PointX_DisplayName, "Point X (Float)");
            AttributeText.Add(aLang.ZN_PointY_DisplayName, "Point Y - Base Height (Float)");
            AttributeText.Add(aLang.ZN_PointZ_DisplayName, "Point Z (Float)");
            AttributeText.Add(aLang.ZN_Point_Description, "Polygon point coordinate of the selected point in game units \r\nEach point is 12 bytes (3 floats) in the data buffer ");
        ...
        public static void StartTexts() 
        {
            ...
            Text.Add(eLang.Wizard_Title, "Setup Wizard");
            Text.Add(eLang.Wizard_Step1, "Welcome");
            Text.Add(eLang.Wizard_Step2, "Game Folders");
            Text.Add(eLang.Wizard_Step3, "Tools");
            Text.Add(eLang.Wizard_Back, "<  Back");
            Text.Add(eLang.Wizard_Skip, "Skip setup");
            Text.Add(eLang.Wizard_Next, "Next  >");
            Text.Add(eLang.Wizard_Finish, "Finish  \u2713");
            Text.Add(eLang.Wizard_WelcomeTitle, "Welcome.");
            Text.Add(eLang.Wizard_WelcomeSub, "Personalize the editor \u2014 everything can be changed later in Options.");
            Text.Add(eLang.Wizard_CreditsRights, "All rights belong to JADERLINK.");
            Text.Add(eLang.Wizard_CreditsEdition, "Professionally modified edition by Nawaf.");
            Text.Add(eLang.Wizard_SectionLanguage, "Language");
            Text.Add(eLang.Wizard_SectionAppearance, "Appearance");
            Text.Add(eLang.Wizard_SkyColor, "Sky color");
            Text.Add(eLang.Wizard_CustomPreset, "Custom");
            Text.Add(eLang.Wizard_CustomColorTitle, "Custom color");
            Text.Add(eLang.Wizard_OK, "OK");
            Text.Add(eLang.Wizard_Cancel, "Cancel");
            Text.Add(eLang.Wizard_ThemeSwitch, "Use the darker theme");
            Text.Add(eLang.Wizard_SectionMouse, "Mouse");
            Text.Add(eLang.Wizard_MouseSwitch, "Invert mouse buttons (left camera, right select)");
            Text.Add(eLang.Wizard_FoldersTitle, "Game folders.");
            Text.Add(eLang.Wizard_AutoDetect, "Auto-detect (Steam)");
            Text.Add(eLang.Wizard_DetectFound, "Found: ");
            Text.Add(eLang.Wizard_DetectNotFound, "\"Resident Evil 4\" was not found \u2014 please browse for it.");
            Text.Add(eLang.Wizard_ToolsTitle, "External tools.");
            Text.Add(eLang.Wizard_ToolsSub, "Leave empty to use the bundled tools.");
            Text.Add(eLang.Wizard_ForceReloadSwitch, "Force reload models JSON files after finishing");
            Text.Add(eLang.Wizard_ForceReloadHint, "Reloads every model and JSON database.");
            ...
            Text.Add(eLang.NodeSMX, "Material (SMX)");
            Text.Add(eLang.NodeAVL, "Aev Lock (AVL)");
            Text.Add(eLang.NodeCAM, "Cameras (CAM)");
            Text.Add(eLang.NodeCAM_ZONE, "Camera Trigger Zones (CAM)");
            Text.Add(eLang.NodeRTP, "Enemy Routes (RTP)");
            ...
            Text.Add(eLang.AddNewSMX, "Add new Shadow Material SMX object");
            Text.Add(eLang.AddNewAVL, "Add new Aev Lock AVL object");
            ...
            Text.Add(eLang.DirectorySMX, "SMX File Patch:");
            Text.Add(eLang.DirectoryAVL, "AVL File Patch:");
            ...
            Text.Add(eLang.MoveMode_RTP_PositionPoint, "RTP Node: Squad and Vertical = Position;");
        ...
        public static void StartOthersTexts() 
        {
            ...
            Text.Add(eLang.toolStripMenuItemNewSMX, "New Shadow Material SMX File (UHD)");
            Text.Add(eLang.toolStripMenuItemNewAVL, "New Aev Lock AVL File (Little Endian)");
            ...
            Text.Add(eLang.toolStripMenuItemOpenSMX, "Open Shadow Material SMX File (UHD)");
            Text.Add(eLang.toolStripMenuItemOpenAVL, "Open Aev Lock AVL File (Little Endian)");
            ...
            Text.Add(eLang.toolStripMenuItemSaveSMX, "Save Shadow Material SMX File");
            Text.Add(eLang.toolStripMenuItemSaveAVL, "Save Aev Lock AVL File");
            ...
            Text.Add(eLang.toolStripMenuItemSaveAsSMX, "Save As Shadow Material SMX File");
            Text.Add(eLang.toolStripMenuItemSaveAsAVL, "Save As Aev Lock AVL File");
            ...
            Text.Add(eLang.toolStripMenuItemClearSMX, "Clear Shadow Material SMX List");
            Text.Add(eLang.toolStripMenuItemClearAVL, "Clear Aev Lock AVL List");
            ...
            Text.Add(eLang.toolStripMenuItemHideFileCAM, "Hide Cameras .CAM");
            Text.Add(eLang.toolStripMenuItemHideFileCAM_ZONE, "Hide Camera Zones .CAM");
            Text.Add(eLang.toolStripMenuItemHideFileRTP, "Hide Enemy Routes .RTP");
            ...
            Text.Add(eLang.openFileDialogSMX, "Open Shadow Material SMX File");
            Text.Add(eLang.openFileDialogAVL, "Open Aev Lock AVL File");
            ...
            Text.Add(eLang.saveFileDialogSMX, "Save Shadow Material SMX File");
            Text.Add(eLang.saveFileDialogAVL, "Save Aev Lock AVL File");
            ...
            Text.Add(eLang.OptionsTabNumbers, "Numbers & Rotation");
