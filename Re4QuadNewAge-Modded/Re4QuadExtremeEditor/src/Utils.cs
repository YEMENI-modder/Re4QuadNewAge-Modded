...
namespace Re4QuadExtremeEditor.src
{
    ...
    {        
        ...
        public static void StartCreateNodes() 
        {
            ...
            DataBase.NodeSMX = new NewAge_SMX_NodeGroup
            {
                Group = GroupType.SMX,
                Text = Lang.GetText(eLang.NodeSMX),
                Name = Consts.NodeSMX,
                ForeColor = Globals.NodeColorSMX,
                NodeFont = Globals.TreeNodeFontText
            };

            DataBase.NodeAVL = new NewAge_AVL_NodeGroup
            {
                Group = GroupType.AVL,
                Text = Lang.GetText(eLang.NodeAVL),
                Name = Consts.NodeAVL,
                ForeColor = Globals.NodeColorAVL,
                NodeFont = Globals.TreeNodeFontText
            };

            DataBase.NodeCAM = new NewAge_CAM_NodeGroup
            {
                Group = GroupType.CAM,
                Text = Lang.GetText(eLang.NodeCAM),
                Name = Consts.NodeCAM,
                ForeColor = Globals.NodeColorCAM,
                NodeFont = Globals.TreeNodeFontText
            };

            DataBase.NodeCAM_Zone = new NewAge_CAM_Zone_NodeGroup
            {
                Group = GroupType.CAM_ZONE,
                Text = Lang.GetText(eLang.NodeCAM_ZONE),
                Name = Consts.NodeCAM_ZONE,
                ForeColor = Globals.NodeColorCAM_ZONE,
                NodeFont = Globals.TreeNodeFontText
            };

            DataBase.NodeRTP = new NewAge_RTP_NodeGroup
            {
                Group = GroupType.RTP,
                Text = Lang.GetText(eLang.NodeRTP),
                Name = Consts.NodeRTP,
                ForeColor = Globals.NodeColorRTP,
                NodeFont = Globals.TreeNodeFontText
            };
        ...
        public static void UpdateNodeTextsLive()
        {
            SetNodeTextLive(DataBase.NodeESL, eLang.NodeESL);
            SetNodeTextLive(DataBase.NodeETS, eLang.NodeETS);
            SetNodeTextLive(DataBase.NodeITA, eLang.NodeITA);
            SetNodeTextLive(DataBase.NodeAEV, eLang.NodeAEV);
            SetNodeTextLive(DataBase.NodeEXTRAS, eLang.NodeEXTRAS);
            SetNodeTextLive(DataBase.NodeDSE, eLang.NodeDSE);
            SetNodeTextLive(DataBase.NodeSMX, eLang.NodeSMX);
            SetNodeTextLive(DataBase.NodeAVL, eLang.NodeAVL);
            SetNodeTextLive(DataBase.NodeCAM, eLang.NodeCAM);
            SetNodeTextLive(DataBase.NodeCAM_Zone, eLang.NodeCAM_ZONE);
            SetNodeTextLive(DataBase.NodeRTP, eLang.NodeRTP);
            SetNodeTextLive(DataBase.NodeEMI, eLang.NodeEMI);
            SetNodeTextLive(DataBase.NodeSAR, eLang.NodeSAR);
            SetNodeTextLive(DataBase.NodeEAR, eLang.NodeEAR);
            SetNodeTextLive(DataBase.NodeESE, eLang.NodeESE);
            SetNodeTextLive(DataBase.NodeFSE, eLang.NodeFSE);
            SetNodeTextLive(DataBase.NodeQuadCustom, eLang.NodeQuadCustom);
            SetNodeTextLive(DataBase.NodeLIT_Groups, eLang.NodeLIT_GROUPS);
            SetNodeTextLive(DataBase.NodeLIT_Entrys, eLang.NodeLIT_ENTRYS);
            SetNodeTextLive(DataBase.NodeEFF_Table0, eLang.NodeEFF_Table0);
            SetNodeTextLive(DataBase.NodeEFF_Table1, eLang.NodeEFF_Table1);
            SetNodeTextLive(DataBase.NodeEFF_Table2, eLang.NodeEFF_Table2);
            SetNodeTextLive(DataBase.NodeEFF_Table3, eLang.NodeEFF_Table3);
            SetNodeTextLive(DataBase.NodeEFF_Table4, eLang.NodeEFF_Table4);
            SetNodeTextLive(DataBase.NodeEFF_Table6, eLang.NodeEFF_Table6);
            SetNodeTextLive(DataBase.NodeEFF_Table7_Effect_0, eLang.NodeEFF_Table7_Effect_0);
            SetNodeTextLive(DataBase.NodeEFF_Table8_Effect_1, eLang.NodeEFF_Table8_Effect_1);
            SetNodeTextLive(DataBase.NodeEFF_EffectEntry, eLang.NodeEFF_EffectEntry);
            SetNodeTextLive(DataBase.NodeEFF_Table9, eLang.NodeEFF_Table9);
        }

        private static void SetNodeTextLive(System.Windows.Forms.TreeNode node, eLang id)
        {
            if (node != null) { node.Text = Lang.GetText(id); }
        }
        ...
        public static List<RoomInfo> LoadRoomInfoList()
        {
            ...
            try
            {
                ...
                if (Directory.Exists(myDirectory))
                {
                    ...
                    var roomListObjs = new List<RoomListObj>();
                    ...
                    foreach (var item in roomListObjs)
                    {
                        ...
                        if (Directory.Exists(subDirectory))
                        {
                            ...
                            var roomModelDict = new Dictionary<string, RoomModel>();
        ...
        /// <summary>
        /// Preloads all data needed by SelectRoomForm and OptionsForm so their
        /// first open is instant.  Called during startup (behind splash screen).
        /// </summary>
        public static void StartPreloadFormCaches()
        {
            // ---- SelectRoomForm ----
            DataBase.CachedRoomInfoList = LoadRoomInfoList();

            // ---- OptionsForm ----
            DataBase.CachedEnemiesLists    = PreloadEnemiesLists();
            DataBase.CachedEtcModelsLists  = PreloadEtcModelsLists();
            DataBase.CachedItemsLists      = PreloadItemsLists();
            DataBase.CachedQuadCustomLists = PreloadQuadCustomLists();
            DataBase.CachedLangList        = PreloadLangList();
        }

        // ------------------------------------------------------------------
        //  OptionsForm helpers  (mirror of the private Get*() methods there)
        // ------------------------------------------------------------------

        private static ObjectInfoList[] PreloadEnemiesLists()
        {
            var lists = new List<ObjectInfoList>();
            ...
            foreach (var f in files)
            {
                try
                {
                    var file = ObjectInfoListFile.ParseFromFileForOptions(f, Consts.NameEnemiesList);
            ...
            var _default = new ObjectInfoList(
                Consts.DefaultEnemiesListFileDirectory, "Default List", "null",
                new Dictionary<ushort, ObjectInfo>());
            if (!lists.Contains(_default))
                lists.Add(_default);

            return lists.ToArray();
        }

        private static ObjectInfoList[] PreloadEtcModelsLists()
        {
            var lists = new List<ObjectInfoList>();
            ...
            foreach (var f in files)
            {
                try
                {
                    var file = ObjectInfoListFile.ParseFromFileForOptions(f, Consts.NameEtcModelsList);
                    if (file != null && !lists.Contains(file))
                        lists.Add(file);
                }
                catch { }
            ...
            var _default = new ObjectInfoList(
                Consts.DefaultEtcModelsListFileDirectory, "Default List", "null",
                new Dictionary<ushort, ObjectInfo>());
            if (!lists.Contains(_default))
                lists.Add(_default);
        ...
        private static ObjectInfoList[] PreloadItemsLists()
        {
            var lists = new List<ObjectInfoList>();
            ...
            foreach (var f in files)
            {
                try
                {
                    var file = ObjectInfoListFile.ParseFromFileForOptions(f, Consts.NameItemsList);
                    if (file != null && !lists.Contains(file))
                        lists.Add(file);
                }
                catch { }
            ...
            var _default = new ObjectInfoList(
                Consts.DefaultItemsListFileDirectory, "Default List", "null",
                new Dictionary<ushort, ObjectInfo>());
            if (!lists.Contains(_default))
                lists.Add(_default);
        ...
        private static QuadCustomInfoList[] PreloadQuadCustomLists()
        {
            var lists = new List<QuadCustomInfoList>();
            ...
            foreach (var f in files)
            {
                try
                {
                    var file = QuadCustomInfoListFile.ParseFromFileForOptions(f);
                    if (file != null && !lists.Contains(file))
                        lists.Add(file);
                }
                catch { }
            ...
            var _default = new QuadCustomInfoList(
                Consts.DefaultQuadCustomModelsListFileDirectory, "Default List", "null",
                new Dictionary<uint, QuadCustomInfo>());
            if (!lists.Contains(_default))
                lists.Add(_default);
        ...
        private static LangObjForList[] PreloadLangList()
        {
            var list = new List<LangObjForList>();
            ...
            foreach (var f in files)
            {
                try
                {
                    var file = LangFile.ParseFromFileForList(f);
