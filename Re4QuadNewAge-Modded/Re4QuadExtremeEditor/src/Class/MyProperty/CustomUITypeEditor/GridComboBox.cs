...
namespace Re4QuadExtremeEditor.src.Class.MyProperty.CustomUITypeEditor
{
    public abstract class GridComboBox : UITypeEditor
    {
		...
		public GridComboBox()
		{
			...
			_listBox.HandleCreated += (sender, e) => ApplyDarkListBoxTheme();
			_listBox.VisibleChanged += (sender, e) =>
			{
				if (_listBox.Visible)
					ApplyDarkListBoxTheme();
			};
		...
		private void PopulateListBox(ITypeDescriptorContext context, object currentValue)
		{
			...
			// Rebuild the list whenever the drop-down opens. UITypeEditor instances are
			// reused by the PropertyGrid, so retaining the first list hides entries that
			// were added later in the current session.
		...
		// PropertyGrid creates this ListBox lazily inside its drop-down window, after
		// the form-level dark-theme pass. Theme it at creation/show time so its native
