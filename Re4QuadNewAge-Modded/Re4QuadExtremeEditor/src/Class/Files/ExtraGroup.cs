...
{  /// <summary>
   ...
   /// </summary>
    public class ExtraGroup
    {
        ...
        {
            AssociationObj assoc;
            if (AssociationList.TryGetValue(ID, out assoc))
            {
                if (assoc.FileFormat == SpecialFileFormat.AEV)
                {
                    return "Extra: " + TextSubPart(DataBase.NodeAEV.PropertyMethods, assoc.LineID, assoc.SubObjID);
                }
                else if (assoc.FileFormat == SpecialFileFormat.ITA)
                {
                    return "Extra: " + TextSubPart(DataBase.NodeITA.PropertyMethods, assoc.LineID, assoc.SubObjID) + " (From ITA)";
                }
            }
        ...
        public Color GetNodeColor(ushort ID)
        {
            ...
            if (AssociationList.TryGetValue(ID, out assoc))
            {
                ...
                {
                    bool isWarpDoor = false;
                    if (assoc.FileFormat == SpecialFileFormat.AEV && DataBase.FileAEV != null && DataBase.FileAEV.Lines.ContainsKey(assoc.LineID))
                        isWarpDoor = DataBase.FileAEV.Methods.GetSpecialType(assoc.LineID) == SpecialType.T01_WarpDoor;
                    else if (assoc.FileFormat == SpecialFileFormat.ITA && DataBase.FileITA != null && DataBase.FileITA.Lines.ContainsKey(assoc.LineID))
                        isWarpDoor = DataBase.FileITA.Methods.GetSpecialType(assoc.LineID) == SpecialType.T01_WarpDoor;
                    if (isWarpDoor) return Globals.NodeColorHided;
                }
        ...
        {
            AssociationObj assoc;
            if (!AssociationList.TryGetValue(ID, out assoc)) return Vector3.Zero;
            if (assoc.FileFormat == SpecialFileFormat.AEV)
            {
                return GetObjPostion_ToCamera_SubPart(DataBase.NodeAEV.ExtrasMethodsForGL, assoc.LineID, assoc.SubObjID);
            }
            else if (assoc.FileFormat == SpecialFileFormat.ITA)
            {
                return GetObjPostion_ToCamera_SubPart(DataBase.NodeITA.ExtrasMethodsForGL, assoc.LineID, assoc.SubObjID);
            }
            return Vector3.Zero;
        }
        ...
        {
            AssociationObj assoc;
            if (!AssociationList.TryGetValue(ID, out assoc)) return null;
            if (assoc.FileFormat == SpecialFileFormat.AEV)
            {
                return GetObjPostion_ToMove_General_SubPart(DataBase.NodeAEV.PropertyMethods, assoc.LineID, assoc.SubObjID);
            }
            else if (assoc.FileFormat == SpecialFileFormat.ITA)
            {
                return GetObjPostion_ToMove_General_SubPart(DataBase.NodeITA.PropertyMethods, assoc.LineID, assoc.SubObjID);
            }
            return null;       
        }
        ...
        {
            AssociationObj assoc;
            if (!AssociationList.TryGetValue(ID, out assoc)) return;
            if (assoc.FileFormat == SpecialFileFormat.AEV)
            {
                SetObjPostion_ToMove_General_SubPart(DataBase.NodeAEV.PropertyMethods, assoc.LineID, assoc.SubObjID, value);
            }
            else if (assoc.FileFormat == SpecialFileFormat.ITA)
            {
                SetObjPostion_ToMove_General_SubPart(DataBase.NodeITA.PropertyMethods, assoc.LineID, assoc.SubObjID,  value);
            }
        }
        ...
        private Vector3[] GetObjRotationAngles_ToMove(ushort ID) 
        {
            ...
            if (assoc.FileFormat == SpecialFileFormat.AEV)
            {
                return GetObjRotarionAngles_ToMove_SubPart(DataBase.NodeAEV.PropertyMethods, assoc.LineID);
            }
            else if (assoc.FileFormat == SpecialFileFormat.ITA)
            {
                return GetObjRotarionAngles_ToMove_SubPart(DataBase.NodeITA.PropertyMethods, assoc.LineID);
            }
            return null;
        }
        ...
        private void SetObjRotationAngles_ToMove(ushort ID, Vector3[] value) 
        {
            ...
            if (assoc.FileFormat == SpecialFileFormat.AEV)
            {
                SetObjRotarionAngles_ToMove_SubPart(DataBase.NodeAEV.PropertyMethods, assoc.LineID, value);
            }
            else if (assoc.FileFormat == SpecialFileFormat.ITA)
            {
                SetObjRotarionAngles_ToMove_SubPart(DataBase.NodeITA.PropertyMethods, assoc.LineID, value);
            }
        }
        ...
        public bool IsEnemyLinkedWithIta(ushort enemyID)
        {
            RefInteractionTypeValue[] links = GetRefInteractionTypeValue(RefInteractionType.Enemy, enemyID);
            if (links == null)
            {
                return false;
            }
            foreach (RefInteractionTypeValue link in links)
            {
                if (link.FileFormat == SpecialFileFormat.ITA)
                {
                    return true;
                }
            }
