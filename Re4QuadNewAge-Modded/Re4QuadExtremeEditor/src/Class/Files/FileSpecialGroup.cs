    ...
    public class FileSpecialGroup : BaseTriggerZoneGroup
    {
        ...
        public string GetNodeText(ushort ID)
        {
            ...
            else if (Lines.ContainsKey(ID))
            {
                ...
                {
                    byte enemyOrderID = ReturnRefInteractionIndex(ID);
                    r += " - Associated Enemy ID: 0x" + enemyOrderID.ToString("X2");

                    // For ITA entries, also show the actual ESL enemy name so linked entries
                    // are easy to identify and less likely to be changed accidentally.
                    if (GetSpecialFileFormat == SpecialFileFormat.ITA &&
                        DataBase.FileESL != null && DataBase.FileESL.Lines.ContainsKey(enemyOrderID))
                    {
                        ...
                        if (DataBase.EnemiesIDs != null && DataBase.EnemiesIDs.List.ContainsKey(enemyModelID) &&
                            !(enemyModelIDHex[2] == 'F' && enemyModelIDHex[3] == 'F'))
                        {
                            enemyName = DataBase.EnemiesIDs.List[enemyModelID].Name;
                        }
                        else if (DataBase.EnemiesIDs != null)
                        {
                            ...
                            if (DataBase.EnemiesIDs.List.ContainsKey(enemyModelIDFF) && enemyModelIDFF != 0xFFFF)
                            {
                                enemyName = DataBase.EnemiesIDs.List[enemyModelIDFF].Name;
                            }
                        }
        ...
        private Vector3[] GetObjPostion_ToMove_General(ushort ID)
        {
            ...
            // Always carry the entry's real position so camera moves (Get)
            // relocate the ITA/AEV itself together with its trigger box.
