...
{
    /// <summary>
    /// Classe responsavel por toda a manipula�?o de carregar e salvar arquivos
    /// </summary>
    public static class FileManager
    {
        ...
        {
            RecentFiles.Note("ESL", file != null ? file.Name : null);
            FileEnemyEslGroup esl = new FileEnemyEslGroup();
            const int recordSize = 32;
            const int maxRecords = 256;
            int totalRecords = Math.Min((int)fileInfo.Length / recordSize, maxRecords);
            byte[] bulkBuffer = new byte[totalRecords * recordSize];
            file.Read(bulkBuffer, 0, bulkBuffer.Length);
            for (ushort i = 0; i < totalRecords; i++)
            {
                byte[] res = new byte[recordSize];
                Buffer.BlockCopy(bulkBuffer, i * recordSize, res, 0, recordSize);
                esl.Lines.Add(i, res);
            }
            for (ushort i = (ushort)totalRecords; i < maxRecords; i++)
            {
                esl.Lines.Add(i, new byte[recordSize]);
            }
            ...
            Object3D[] nodes = new Object3D[256];
            for (ushort iN = 0; iN < 256; iN++)
            {
                nodes[iN] = Object3D.CreateNewInstance(GroupType.ESL, iN);
            }
            DataBase.NodeESL.Nodes.AddRange(nodes);
            DataBase.NodeESL.Expand();
        }

        public static void LoadFileETS_2007_PS2(FileStream file, FileInfo fileInfo) 
        {
            RecentFiles.Note("ETS_2007", file != null ? file.Name : null);
            ...
            const int etsRecordSize = 64;
            byte[] bulkBuffer = new byte[Amount * etsRecordSize];
            file.Read(bulkBuffer, 0, bulkBuffer.Length);

            for (ushort i = 0; i < Amount; i++)
            {
                byte[] res = new byte[etsRecordSize];
                Buffer.BlockCopy(bulkBuffer, i * etsRecordSize, res, 0, etsRecordSize);
                ets.Lines.Add(i, res);
            }
            ...
            Object3D[] nodes = new Object3D[Amount];
            for (ushort iN = 0; iN < Amount; iN++)
            {
                nodes[iN] = Object3D.CreateNewInstance(GroupType.ETS, iN);
            }
            DataBase.NodeETS.Nodes.AddRange(nodes);
            DataBase.NodeETS.Expand();
        }

        public static void LoadFileETS_UHD(FileStream file, FileInfo fileInfo)
        {
            RecentFiles.Note("ETS_UHD", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("ITA_2007", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("ITA_UHD", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("AEV_2007", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("AEV_UHD", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("DSE", file != null ? file.Name : null);
        ...
        public static void LoadFileSMX(FileStream file, FileInfo fileInfo)
        {
            RecentFiles.Note("SMX", file != null ? file.Name : null);
            File_SMX_Group smx = new File_SMX_Group();
            byte[] header = new byte[16];
            int offset = file.Read(header, 0, 16);
            byte magic = header[0x00];
            byte Amount = header[0x01];

            if (magic != 0x10)
            {
                throw new Exception("Invalid SMX magic byte: expected 0x10, got 0x" + magic.ToString("X2"));
            }

            if (Amount > Consts.AmountLimitSMX)
            {
                Amount = (byte)Consts.AmountLimitSMX;
            }

            ushort i = 0;
            for (; i < Amount; i++)
            {
                byte[] res = new byte[File_SMX_Group.SMX_RECORD_SIZE];
                offset = file.Read(res, 0, File_SMX_Group.SMX_RECORD_SIZE);
                smx.Lines.Add(i, res);
            ...
            if (i < Amount)
            {
                for (; i < Amount; i++)
                {
                    smx.Lines.Add(i, new byte[File_SMX_Group.SMX_RECORD_SIZE]);
                }
            }

            long expectedEnd = 16 + (Amount * File_SMX_Group.SMX_RECORD_SIZE);
            if (fileInfo.Length > expectedEnd)
            {
                smx.EndFile = new byte[fileInfo.Length - expectedEnd];
                file.Read(smx.EndFile, 0, (int)(fileInfo.Length - expectedEnd));
            }

            smx.IdForNewLine = i;

            DataBase.FileSMX = null;
            DataBase.FileSMX = smx;

            DataBase.NodeSMX.Nodes.Clear();
            DataBase.NodeSMX.PropertyMethods = DataBase.FileSMX.Methods;
            DataBase.NodeSMX.DisplayMethods = DataBase.FileSMX.DisplayMethods;
            DataBase.NodeSMX.MoveMethods = DataBase.FileSMX.MoveMethods;
            DataBase.NodeSMX.ChangeAmountMethods = DataBase.FileSMX.ChangeAmountMethods;
            List<Object3D> nodes = new List<Object3D>();
            for (ushort iN = 0; iN < Amount; iN++)
            {
                Object3D o = Object3D.CreateNewInstance(GroupType.SMX, iN);
                nodes.Add(o);
            }
            DataBase.NodeSMX.Nodes.AddRange(nodes.ToArray());
            DataBase.NodeSMX.Expand();
        }

        public static void LoadFileAVL(FileStream file, FileInfo fileInfo)
        {
            RecentFiles.Note("AVL", file != null ? file.Name : null);
            File_AVL_Group avl = new File_AVL_Group();
            byte[] all = new byte[fileInfo.Length];
            file.Read(all, 0, (int)fileInfo.Length);

            byte[] marker = Encoding.ASCII.GetBytes("Number of Aev--");
            List<int> starts = new List<int>();
            for (int i = 0; i <= all.Length - marker.Length; i++)
            {
                bool ok = true;
                for (int j = 0; j < marker.Length; j++)
                {
                    if (all[i + j] != marker[j]) { ok = false; break; }
                }
                if (ok)
                {
                    starts.Add(i);
            ...
            else
            {
                if (starts[0] > 0)
                {
                    avl.StartFile = new byte[starts[0]];
                    Array.Copy(all, 0, avl.StartFile, 0, starts[0]);
                }

                ushort count = (ushort)Math.Min(starts.Count, Consts.AmountLimitAVL);
                for (ushort s = 0; s < count; s++)
                {
                    int begin = starts[s];
                    int end = (s + 1 < starts.Count) ? starts[s + 1] : all.Length;
                    int len = end - begin;
                    byte[] rec = new byte[len];
                    Array.Copy(all, begin, rec, 0, len);
                    avl.Lines.Add(s, rec);
                }
            }

            avl.IdForNewLine = (ushort)avl.Lines.Count;

            DataBase.FileAVL = null;
            DataBase.FileAVL = avl;

            DataBase.NodeAVL.Nodes.Clear();
            DataBase.NodeAVL.PropertyMethods = DataBase.FileAVL.Methods;
            DataBase.NodeAVL.DisplayMethods = DataBase.FileAVL.DisplayMethods;
            DataBase.NodeAVL.MoveMethods = DataBase.FileAVL.MoveMethods;
            DataBase.NodeAVL.ChangeAmountMethods = DataBase.FileAVL.ChangeAmountMethods;
            List<Object3D> nodesAvl = new List<Object3D>();
            for (ushort iN = 0; iN < avl.Lines.Count; iN++)
            {
                Object3D o = Object3D.CreateNewInstance(GroupType.AVL, iN);
                nodesAvl.Add(o);
            }
            DataBase.NodeAVL.Nodes.AddRange(nodesAvl.ToArray());
            DataBase.NodeAVL.Expand();
        }

        public static void LoadFileFSE(FileStream file, FileInfo fileInfo)
        {
            RecentFiles.Note("FSE", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("SAR", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("EAR", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("EMI_2007", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("EMI_UHD", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("ESE_2007", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("ESE_UHD", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("QUAD", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note(endianness == Endianness.BigEndian ? "EFFBLOB_BE" : "EFFBLOB_LE", file != null ? file.Name : null);
        ...
        {
            RecentFiles.Note("CAM", file != null ? file.Name : null);
            if (version == IsRe4Version.BIG_ENDIAN || version == IsRe4Version.V2007PS2)
            {
                throw new NotSupportedException("CAM files of this version are not supported, use the UHD CAM file.");
            }
            File_CAM_Group cam = new File_CAM_Group();
            ...
            {
                int keys = 0, pts = 0;
                foreach (var c in cam.Cameras.Values) { keys += c.Positions.Count; }
                foreach (var z in cam.Zones.Values) { pts += z.Points.Count; }
                string logPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Re4Quad_error_log.txt");
                File.AppendAllText(logPath,
                    "[" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "] CAM LOAD INFO: "
                    + Path.GetFileName(file.Name)
                    + " size=" + all.Length
                    + " cameras=" + cam.Cameras.Count
                    + " keys=" + keys
                    + " zones=" + cam.Zones.Count
                    + " zonePts=" + pts + "\r\n");
            }
            catch
            {
            ...
            DataBase.FileCAM = null;
            DataBase.FileCAM = cam;

            DataBase.NodeCAM.Nodes.Clear();
            DataBase.NodeCAM.PropertyMethods = DataBase.FileCAM.Methods;
            DataBase.NodeCAM.DisplayMethods = DataBase.FileCAM.DisplayMethods;
            DataBase.NodeCAM.MoveMethods = DataBase.FileCAM.MoveMethods;
            DataBase.NodeCAM.ChangeAmountMethods = DataBase.FileCAM.ChangeAmountMethods;

            DataBase.NodeCAM_Zone.Nodes.Clear();
            DataBase.NodeCAM_Zone.PropertyMethods = DataBase.FileCAM.ZoneMethods;
            DataBase.NodeCAM_Zone.DisplayMethods = DataBase.FileCAM.ZoneDisplayMethods;
            DataBase.NodeCAM_Zone.MoveMethods = DataBase.FileCAM.ZoneMoveMethods;
            DataBase.NodeCAM_Zone.ChangeAmountMethods = DataBase.FileCAM.ZoneChangeAmountMethods;
            DataBase.NodeCAM.MethodsForGL = DataBase.FileCAM.MethodsForGL;
            DataBase.NodeCAM_Zone.MethodsForGL = DataBase.FileCAM.ZoneMethodsForGL;

            cam.RebuildCamNodeList();
            List<Object3D> nodesCam = new List<Object3D>();
            for (ushort iN = 0; iN < cam.CamNodeList.Count; iN++)
            {
                Object3D o = Object3D.CreateNewInstance(GroupType.CAM, iN);
                nodesCam.Add(o);
            }
            DataBase.NodeCAM.Nodes.AddRange(nodesCam.ToArray());
            DataBase.NodeCAM.Expand();

            List<Object3D> nodesZone = new List<Object3D>();
            for (ushort iN = 0; iN < cam.Zones.Count; iN++)
            {
                Object3D o = Object3D.CreateNewInstance(GroupType.CAM_ZONE, iN);
                nodesZone.Add(o);
            }
            DataBase.NodeCAM_Zone.Nodes.AddRange(nodesZone.ToArray());
            DataBase.NodeCAM_Zone.Expand();
        }
...

            {
                ...
                for (; i < Amount; i++)
                {
                    if (groupOffsets[i] != 0)
                    {
                        ...
                        //esse campo na edi�?o fica sempre zero
                        Group[4] = 0;
                        Group[5] = 0;
                        Group[6] = 0;
                        Group[7] = 0;
        ...
        {
            RecentFiles.Note("ITA_PS4NS", file != null ? file.Name : null);
        ...
        public static void ClearSMX()
        {
            DataBase.NodeSMX.Nodes.Clear();
            DataBase.NodeSMX.PropertyMethods = null;
            DataBase.NodeSMX.DisplayMethods = null;
            DataBase.NodeSMX.MoveMethods = null;
            DataBase.NodeSMX.ChangeAmountMethods = null;
            DataBase.FileSMX = null;
        }

        public static void ClearAVL()
        {
            DataBase.NodeAVL.Nodes.Clear();
            DataBase.NodeAVL.PropertyMethods = null;
            DataBase.NodeAVL.DisplayMethods = null;
            DataBase.NodeAVL.MoveMethods = null;
            DataBase.NodeAVL.ChangeAmountMethods = null;
            DataBase.FileAVL = null;
        }
        ...
        {
            DataBase.NodeCAM.Nodes.Clear();
            DataBase.NodeCAM.PropertyMethods = null;
            DataBase.NodeCAM.DisplayMethods = null;
            DataBase.NodeCAM.MoveMethods = null;
            DataBase.NodeCAM.ChangeAmountMethods = null;
            DataBase.NodeCAM.MethodsForGL = null;
            DataBase.NodeCAM_Zone.Nodes.Clear();
            DataBase.NodeCAM_Zone.PropertyMethods = null;
            DataBase.NodeCAM_Zone.DisplayMethods = null;
            DataBase.NodeCAM_Zone.MoveMethods = null;
            DataBase.NodeCAM_Zone.ChangeAmountMethods = null;
            DataBase.NodeCAM_Zone.MethodsForGL = null;
            DataBase.FileCAM = null;
        }

        public static void ClearRTP()
        {
            DataBase.NodeRTP.Nodes.Clear();
            DataBase.NodeRTP.MethodsForGL = null;
            DataBase.NodeRTP.PropertyMethods = null;
            DataBase.NodeRTP.DisplayMethods = null;
            DataBase.NodeRTP.MoveMethods = null;
            DataBase.NodeRTP.ChangeAmountMethods = null;
            DataBase.FileRTP = null;
        }

        public static void SaveFileCAM(FileStream stream)
        {
            if (DataBase.FileCAM != null)
            {
                DataBase.FileCAM.WriteTo(stream);
            }
        }

        public static void SaveFileRTP(FileStream stream)
        {
        ...
        public static void SaveFileSMX(FileStream stream)
        {
            if (DataBase.FileSMX != null && DataBase.FileSMX.Lines != null)
            {
                byte[] header = new byte[16];
                header[0x00] = 0x10;
                header[0x01] = (byte)DataBase.FileSMX.Lines.Count;

                stream.Write(header, 0, 16);

                var nodes = DataBase.NodeSMX.Nodes.Cast<Object3D>();
                ushort[] Order = (from obj in nodes select obj.ObjLineRef).ToArray();

                for (int i = 0; i < Order.Length; i++)
                {
                    byte[] b = DataBase.FileSMX.Lines[Order[i]];
                    stream.Write(b, 0, b.Length);
                }

                if (DataBase.FileSMX.EndFile != null && DataBase.FileSMX.EndFile.Length > 0)
                {
                    stream.Write(DataBase.FileSMX.EndFile, 0, DataBase.FileSMX.EndFile.Length);
                }
            }
        }

        public static void SaveFileAVL(FileStream stream)
        {
            if (DataBase.FileAVL != null && DataBase.FileAVL.Lines != null)
            {
                if (DataBase.FileAVL.StartFile != null && DataBase.FileAVL.StartFile.Length > 0)
                {
                    stream.Write(DataBase.FileAVL.StartFile, 0, DataBase.FileAVL.StartFile.Length);
                }

                var nodes = DataBase.NodeAVL.Nodes.Cast<Object3D>();
                ushort[] Order = (from obj in nodes select obj.ObjLineRef).ToArray();

                for (int i = 0; i < Order.Length; i++)
                {
                    byte[] b = DataBase.FileAVL.Lines[Order[i]];
                    stream.Write(b, 0, b.Length);
                }
            }
        }
        ...
        {
            // todos os dois s?o 64 bytes de tamanho 
            // UHD 0x14 at� final, tem nada zempre 00
            // classic 0x18 at� final, tem nada zempre 00
            ...
            // a parte zerada, acho que � padding, ent?o estou ignorando.
            return res;
        }
