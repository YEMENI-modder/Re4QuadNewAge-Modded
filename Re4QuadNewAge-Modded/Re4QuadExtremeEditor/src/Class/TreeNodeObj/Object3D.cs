...
namespace Re4QuadExtremeEditor.src.Class.TreeNodeObj
{
    ...
    public class Object3D : TreeNode, NsMultiselectTreeView.IAltNode, NsMultiselectTreeView.IFastNode, IEquatable<Object3D>, NsCamera.IObject3D
    {
        ...
        /// <summary>
        /// Move the object so its camera-space position becomes the requested position.
        /// RE4 commonly stores positions at 100x the renderer/camera coordinate scale.
        /// Trigger zones are translated as a whole to preserve their shape.
        /// </summary>
        public void SetObjPosition_ToCamera(Vector3 position)
        {
            Vector3 current = GetObjPosition_ToCamera();
            Vector3[] values = GetObjPostion_ToMove_General();
            if (values == null || values.Length == 0) return;

            float scale = DetectCameraToMoveScale(current, values);
            Vector3 delta = (position - current) * scale;

            if (values.Length >= 7 && GetTriggerZoneCategory() != TriggerZoneCategory.Disable)
            {
                // Move the entry's own position together with its trigger box
                // (the box corners below only cover the zone geometry).
                values[0] += delta;
                for (int i = 1; i <= 4; i++)
                {
                    values[i].X += delta.X;
            ...
            SetObjPostion_ToMove_General(values);
        }

        private static float DetectCameraToMoveScale(Vector3 cameraPosition, Vector3[] values)
        {
            Vector3 raw = values[0];
            if (values.Length >= 7) raw = values[6];

            float[] c = { cameraPosition.X, cameraPosition.Y, cameraPosition.Z };
            float[] r = { raw.X, raw.Y, raw.Z };
            bool saw100 = false;

            for (int i = 0; i < 3; i++)
            {
                if (Math.Abs(c[i]) < 0.01f || float.IsNaN(c[i]) || float.IsInfinity(c[i])) continue;
                float ratio = r[i] / c[i];
                if (Math.Abs(ratio - 1f) < 0.05f) return 1f;
                if (Math.Abs(ratio - 100f) < 5f) saw100 = true;
            }

            return saw100 ? 100f : 100f;
        }
