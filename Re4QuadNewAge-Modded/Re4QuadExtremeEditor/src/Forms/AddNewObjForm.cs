...
namespace Re4QuadExtremeEditor.src.Forms
{
    public partial class AddNewObjForm : Form
    {
        ...
        public AddNewObjForm()
        {
            ...
            if (DataBase.FileSMX != null && DataBase.FileSMX.Lines.Count < Consts.AmountLimitSMX)
            {
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.SMX, 0x00, Lang.GetText(eLang.AddNewSMX)));
            }
            if (DataBase.FileAVL != null && DataBase.FileAVL.Lines.Count < Consts.AmountLimitAVL)
            {
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.AVL, 0x00, Lang.GetText(eLang.AddNewAVL)));
            }
            ...
            if (DataBase.FileCAM != null && DataBase.NodeCAM.Nodes.Count < Consts.AmountLimitCAM)
            {
                // plain default camera
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.CAM, 0xFF, "New CAM"));
                // ready-made presets, same as the "From Template" panel of the CAM tool
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.CAM, 0x00, "Camera - Type 0 Locked Cam (Walk-in, 1 key, FOV 60)"));
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.CAM, 0x01, "Camera - Type 2 Overhead Cam (Walk-in, 2 keys, FOV 65)"));
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.CAM, 0x02, "Camera - Type 6 Inspection Cam (AEV, 2 keys, FOV 50, Speed 0>30)"));
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.CAM, 0x03, "Camera - Type 8 Shoulder Cam (Walk-in, 24 keys, FOV 90)"));
            }
            if (DataBase.FileRTP != null && DataBase.NodeRTP.Nodes.Count < Consts.AmountLimitRTP)
            {
                comboBoxType.Items.Add(new NewEntryObjForListBox(GroupType.RTP, 0x00, "New RTP Node"));
            }
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            if (comboBoxType.SelectedItem is NewEntryObjForListBox gt)
            {
                ...
                if (gt.ID == GroupType.ETS)
                {
                    ...
                    if (nodes.Count > 0) ViewAnim.ShowToast("Added " + nodes.Count + " " + gt.ID);
                ...
                else if (gt.ID == GroupType.SMX)
                {
                    List<Object3D> nodes = new List<Object3D>();
                    for (int i = 0; i < numericUpDownAmount.Value; i++)
                    {
                        if (DataBase.NodeSMX.Nodes.Count + nodes.Count < Consts.AmountLimitSMX)
                        {
                            ushort NewId = DataBase.NodeSMX.ChangeAmountMethods.AddNewLineID(gt.InitType);
                            Object3D o = Object3D.CreateNewInstance(GroupType.SMX, NewId);
                            nodes.Add(o);
                        }
                        else
                        {
                ...
                else if (gt.ID == GroupType.AVL)
                {
                    List<Object3D> nodes = new List<Object3D>();
                    for (int i = 0; i < numericUpDownAmount.Value; i++)
                    {
                        if (DataBase.NodeAVL.Nodes.Count + nodes.Count < Consts.AmountLimitAVL)
                        {
                            ushort NewId = DataBase.NodeAVL.ChangeAmountMethods.AddNewLineID(gt.InitType);
                            Object3D o = Object3D.CreateNewInstance(GroupType.AVL, NewId);
                            nodes.Add(o);
                        }
                        else
                        {
                    ...
                    DataBase.NodeAVL.Nodes.AddRange(nodes.ToArray());
                    if (nodes.Count > 0) ViewAnim.ShowToast("Added " + nodes.Count + " " + gt.ID);
                ...
                else if (gt.ID == GroupType.CAM)
                {
                    // the CAM group builds its own tree nodes (one per
                    // keyframe) and pairs every new camera with its trigger;
                    // appending nodes here would duplicate them
                    int added = 0;
                    for (int i = 0; i < numericUpDownAmount.Value; i++)
                    {
                        if (DataBase.FileCAM != null && DataBase.FileCAM.Cameras.Count < Consts.AmountLimitCAM)
                        {
                            DataBase.NodeCAM.ChangeAmountMethods.AddNewLineID(gt.InitType);
                            added++;
                        }
                        else
                        {
                    ...
                    if (added > 0) ViewAnim.ShowToast("Added " + added + " " + gt.ID);
                    DataBase.NodeCAM.Expand();
                    DataBase.NodeCAM_Zone.Expand();
                }

                else if (gt.ID == GroupType.RTP)
                {
                    List<Object3D> nodes = new List<Object3D>();
                    for (ushort i = 0; i < numericUpDownAmount.Value; i++)
                    {
                        if (DataBase.FileRTP != null && DataBase.FileRTP.Nodes.Count < Consts.AmountLimitRTP)
                        {
                            ushort NewId = DataBase.NodeRTP.ChangeAmountMethods.AddNewLineID(gt.InitType);
                            Object3D o = Object3D.CreateNewInstance(GroupType.RTP, NewId);
                            nodes.Add(o);
                        }
                        else
                        {
                    ...
                    DataBase.NodeRTP.Nodes.AddRange(nodes.ToArray());
                    if (nodes.Count > 0) ViewAnim.ShowToast("Added " + nodes.Count + " " + gt.ID);
