...
// e em https://github.com/opentk/LearnOpenTK/blob/master/Common/Camera.cs
{
    ...
    public class Camera
    {
        ...
        public float OrbitDistance { get { return orbitDistance; } }

        /// <summary>
        /// Set the orbit distance (clamped) and recompute the camera position
        /// around the current orbit target. Used by the smooth focus animation.
        /// </summary>
        public void SetOrbitDistance(float value)
        {
            orbitDistance = MathHelper.Clamp(value, 10.0f, 1000000.0f);
            updateOrbitCamera();
        }
        ...
        /// <summary>
        /// Fully restores the camera from an external state (project load) so
        /// the very next rendered frame already shows the saved view.
        /// In FLY mode yaw/pitch are the source of truth (the look-at target
        /// goes stale while flying), so it is rebuilt from them.
        /// In ORBIT mode position+look-at stay synced, so the orbit parameters
        /// are recovered from that geometry and orbiting continues smoothly.
        /// </summary>
        public void ApplyExternalState(Vector3 position, float yawDegrees, float pitchDegrees, Vector3 lookTarget, int mode)
        {
            camMode = (CameraMode)MathHelper.Clamp(mode, 0, 2);

            if (camMode == CameraMode.ORBIT)
            {
                pos = position;
                lookat = lookTarget;
                Vector3 delta = position - lookTarget;
                float dist = delta.Length;
                if (dist > 0.0001f)
                {
                    orbitDistance = MathHelper.Clamp(dist, 10.0f, 1000000.0f);
                    //inverse of updateOrbitCamera(): recover theta/phi from pos-target
                    orbitPhi = (float)Math.Asin(MathHelper.Clamp(-delta.Y / dist, -1f, 1f));
                    orbitTheta = (float)Math.Atan2(-delta.X, delta.Z);
                }
                UpdateAnglesFromOrbit(); //sync _yaw/_pitch with the restored orbit
                UpdateVectors();
            }
            else
            {
                camMode = CameraMode.FLY;
                pos = position;
                YawDegrees = yawDegrees; //setter already calls UpdateVectors()
                PitchDegrees = pitchDegrees;
                orientateCam(); //rebuild a fresh look-at from pos + front
            }
        ...
        // ---- frame-rate independent movement (amount is already dt-scaled) ----
        public void MoveFront(float amount) { pos += _front * amount; }
        public void MoveBack(float amount) { pos -= _front * amount; }
        public void MoveRight(float amount) { pos += _right * amount; }
        public void MoveLeft(float amount) { pos -= _right * amount; }
        public void MoveUpCam(float amount) { pos += _up * amount; }
        public void MoveDownCam(float amount) { pos -= _up * amount; }

        /// <summary>
        /// Applies buffered mouse-look rotation using the exact FLY-mode math,
        /// so rotation happens once per rendered frame instead of at the
        /// irregular rate Windows delivers WM_MOUSEMOVE messages.
        /// </summary>
        public void ApplyLookDelta(float deltaX, float deltaY)
        {
            const float sensitivity = 0.2f;
            YawDegrees += deltaX * sensitivity;
            PitchDegrees -= deltaY * sensitivity; // reversed since y-coordinates range from bottom to top
        }
        ...
        /// <summary>
        /// Update only the look-at target to the selected object but keep the camera position
        /// This prevents the camera from jumping when the selection changes while in ORBIT mode.
        /// </summary>
        public void UpdateLookAtToSelectedObjectKeepPos()
        {
            ...
            Vector3 objPos = obj.GetObjPosition_ToCamera();
            // Only update lookat to new object position, keep pos (camera location) unchanged
            lookat.X = objPos.X;
            lookat.Y = objPos.Y;
            lookat.Z = objPos.Z;
            // Recompute angles/vectors based on new lookat while preserving pos
            UpdateAnglesFromOrbit();
            UpdateVectors();
            resetMouse = true;
        }
        ...
        /// <summary>
        /// Get = bring the selected object to the current camera position/direction.
        /// The camera itself is not moved.
        /// </summary>
        public bool BringSelectedObjectToCamera()
        {
            if (getSelectedObject == null) return false;
            ...
            obj.SetObjPosition_ToCamera(pos + direction * 50.0f);
            return true;
        }
    ...
    public interface IObject3D
    {
        ...
        void SetObjPosition_ToCamera(Vector3 position);
